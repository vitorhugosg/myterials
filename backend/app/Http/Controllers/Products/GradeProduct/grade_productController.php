    ...
    public function get(Request $request,$idCompany){
        ...
        if ($getCollection = $collection->get($request, $idCompany)) {
            ...
            return [
                'status'=> true,
                'grade_product' =>[
                    'active' => Grade_Product::where('status', 1)->whereIn('collection_id', $idCollections)->get(),
                    'desactive' => Grade_Product::where('status', 0)->whereIn('collection_id', $idCollections)->get()
                ],
            ];
        }else{
            return [

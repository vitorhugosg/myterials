<template>
    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="row">
                <div class="col-md-12">
                    <p class="h5 text-center mb-4">Volume Type Product</p>    
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="mx-auto py-4" style="max-width: 300px">
                        <h3>Add new Volume Type Product</h3>
                        <p class="red">{{errorValidation}}</p>
                        <mdb-input class="grey-text" type="text" v-model="nameAdd" label="Volume Type Product name"/>
...
export default {
    ...
    mounted(){
        this.loader = true
        if(this.$store.getters.getVolumeType  == null){
            this.$http.get(this.$urlAPI + 'products/volume_type/get/'+ this.$route.params.idCompany,{
                "headers":{
                ...
                if(response.data.status){
                    this.active = response.data.volume_type.active;
                    this.desactive = response.data.volume_type.desactive;
                    this.$store.commit('SET_VOLUME_TYPE', response.data.volume_type);
                }else{
    ...
    methods:{
        ...
        activeModal(id,type){
            ...
            if(type){
                this.$http.post(this.$urlAPI + 'products/volume_type/desactive',data,{
                    "headers":{
                        "authorization": "Bearer "+  this.$store.getters.getToken
                    }
                }).then(response =>{
                ...
                this.$http.post(this.$urlAPI + 'products/volume_type/activate',data,{
                    "headers":{
                        "authorization": "Bearer "+  this.$store.getters.getToken
                    }
                }).then(response =>{
        ...
        addMaterial(){
            ...
            this.$http.post(this.$urlAPI + 'products/volume_type/add',data,{
                "headers":{
                    "authorization": "Bearer "+  this.$store.getters.getToken
                }
            }).then(response =>{

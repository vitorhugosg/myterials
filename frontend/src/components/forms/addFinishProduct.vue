<template>
    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="row">
                <div class="col-md-12">
                    <p class="h5 text-center mb-4">Finish Product</p>    
                </div>
            </div>
...
export default {
        ...
        if(this.$store.getters.getFinishProduct  == null){
            this.$http.get(this.$urlAPI + 'products/finish_product/get/'+ this.$route.params.idCompany,{
                ...
                if(response.data.status){
                    ...
                    this.$store.commit('SET_MATERIAL_TYPE', response.data.finish_product);
                }else{
    ...
    methods:{
        ...
        activeModal(id,type){
            ...
            if(type){
                this.$http.post(this.$urlAPI + 'products/finish_product/desactive',data,{
                    "headers":{
                        "authorization": "Bearer "+  this.$store.getters.getToken
                    }
                }).then(response =>{
                    if(response.data.status){
                        this.active = response.data.finish_product.active;
                        this.desactive = response.data.finish_product.desactive;
                        this.$store.commit('SET_FINISH_PRODUCT', response.data.finish_product);
                    }else{
                        this.errorValidation = 'Error get informations for organizations';
                    }
                }).catch(e => {
                ...
                this.$http.post(this.$urlAPI + 'products/finish_product/activate',data,{
                    "headers":{
                        "authorization": "Bearer "+  this.$store.getters.getToken
                    }
                }).then(response =>{

<template>
    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="row">
                <div class="col-md-12">
                    <p class="h5 text-center mb-4">Profile Product</p>    
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="mx-auto py-4" style="max-width: 300px">
                        <h3>Add new Profile Product</h3>
                        <p class="red">{{errorValidation}}</p>
                        <mdb-input class="grey-text" type="text" v-model="nameAdd" label="Profile Product name"/>
...
export default {
    ...
    mounted(){
        this.loader = true
        if(this.$store.getters.getProfileProduct  == null){
            this.$http.get(this.$urlAPI + 'products/profile_product/get/'+ this.$route.params.idCompany,{
                "headers":{
                ...
                if(response.data.status){
                    this.active = response.data.profile_product.active;
                    this.desactive = response.data.profile_product.desactive;
                    this.$store.commit('SET_PROFILE_PRODUCT', response.data.profile_product);
                }else{
    ...
    methods:{
        ...
        activeModal(id,type){
            ...
            if(type){
                this.$http.post(this.$urlAPI + 'products/profile_product/desactive',data,{
                    "headers":{
                        "authorization": "Bearer "+  this.$store.getters.getToken
                    }
                }).then(response =>{
                ...
                this.$http.post(this.$urlAPI + 'products/profile_product/activate',data,{
                    "headers":{
                        "authorization": "Bearer "+  this.$store.getters.getToken
                    }
                }).then(response =>{
                    if(response.data.status){
                        this.active = response.data.profile_product.active;
                        this.desactive = response.data.profile_product.desactive;
                        this.$store.commit('SET_MOISTURE_PRODUCT', response.data.profile_product);
                    }else{
                        this.errorValidation = 'Error get informations for organizations';
                    }
                }).catch(e => {
        ...
        addMaterial(){
            ...
            this.$http.post(this.$urlAPI + 'products/profile_product/add',data,{
                "headers":{
                    "authorization": "Bearer "+  this.$store.getters.getToken
                }
            }).then(response =>{

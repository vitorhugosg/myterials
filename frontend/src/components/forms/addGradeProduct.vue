<template>
    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="row">
                <div class="col-md-12">
                    <p class="h5 text-center mb-4">Grade Product</p>    
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="mx-auto py-4" style="max-width: 300px">
                        <h3>Add new Grade Product</h3>
                        <p class="red">{{errorValidation}}</p>
                        <mdb-input class="grey-text" type="text" v-model="nameAdd" label="Grade Product name"/>
...
export default {
    ...
    mounted(){
        this.loader = true
        if(this.$store.getters.getGradeProduct  == null){
            this.$http.get(this.$urlAPI + 'products/grade_product/get/'+ this.$route.params.idCompany,{
                "headers":{
                ...
                if(response.data.status){
                    this.active = response.data.grade_product.active;
                    this.desactive = response.data.grade_product.desactive;
                    this.$store.commit('SET_GRADE_PRODUCT', response.data.grade_product);
                }else{
            ...
            this.active = this.$store.getters.getGradeProduct.active;
            this.desactive = this.$store.getters.getGradeProduct.desactive;
            this.loader = false;
        }
    },
    computed:{
    ...
    methods:{
        ...
        activeModal(id,type){
            ...
            if(type){
                this.$http.post(this.$urlAPI + 'products/grade_product/desactive',data,{
                    "headers":{
                        "authorization": "Bearer "+  this.$store.getters.getToken
                    }
                }).then(response =>{
                ...
                this.$http.post(this.$urlAPI + 'products/grade_product/activate',data,{
                    "headers":{
                        "authorization": "Bearer "+  this.$store.getters.getToken
                    }
                }).then(response =>{
        ...
        addMaterial(){
            ...
            this.$http.post(this.$urlAPI + 'products/grade_product/add',data,{
                "headers":{
                    "authorization": "Bearer "+  this.$store.getters.getToken
                }
            }).then(response =>{
